...
ID = sys.argv[1] # Select the wind farm from 1 to 19 
row = gdf.loc[gdf['FarmID'] == ID] 
...
if month_end - month_start == 4: #small farms, one simulation for the whole summer
    t_seed = 60 #seeding duration, in days
    name_file = 'simulation_farm_{}_June_September_{}.nc'.format(ID,year)
else : #bigger farms, summer cut in two simulations
    t_seed = 30
    if ID not in [11, 13]:
        name_file = 'simulation_farm_{}_June_September_{}_{}.nc'.format(ID,year,month_start)
    else :
        nb = area * 6 # lower density of particles for the OWF 11 and 13
        name_file = 'simulation_farm_{}_6_part_June_September_{}_{}.nc'.format(ID,year,month_start)
...
for i in range(0,t_seed,3): # seeding during 30 or 60 days every 3 days
    o.seed_from_geopandas(loc, z=-1, number=nb/3, radius=100, time=starttime+dt.timedelta(days=i), origin_marker=ID)
    o.seed_from_geopandas(loc, z=-5, number=nb/3, radius=100, time=starttime+dt.timedelta(days=i), origin_marker=ID)
    o.seed_from_geopandas(loc, z=-10, number=nb/3, radius=100, time=starttime+dt.timedelta(days=i), origin_marker=ID)

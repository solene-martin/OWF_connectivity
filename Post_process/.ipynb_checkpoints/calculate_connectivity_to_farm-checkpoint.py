...
'''
Return a csv file with the connectivity value from either OWF or coastal areas to OWF.
...
year = int(sys.argv[2])
PLD = int(sys.argv[3])
month = int(sys.argv[4]) # accepted value : 0 (whole simulation), 6 (first half, seeding in June) or 7 (second half, seeding in July)

PLD_max = 60

outfol = '/scratch/project_2018610/opendrift_outputs/simulation_15_part_per_km2/seeding_from_farm/'
save_fol = f'/scratch/project_2018610/opendrift_outputs/simulation_15_part_per_km2/connectivity/PD{PLD}/from_farm_to_farm/'
...
ncfile = outfol + file
csvfile = save_fol + file.replace('.nc', f'_PD{PLD}_connectivity.csv')
...
joined = gpd.sjoin(coords[['traj','age_hours','geometry']],poly[['FarmID','geometry']],
